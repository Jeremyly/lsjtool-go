/*
 * @Author: lsjweiyi
 * @Date: 2023-11-22 19:33:58
 * @LastEditors: lsjweiyi
 * @LastEditTime: 2024-03-21 22:07:06
 * @Description: 依赖zap创建日志对象
...
func CreateZapFactory(entry func(zapcore.Entry) error) *zap.Logger {
	...
	appDebug := g.VP.GetBool("AppDebug")
	...
	timePrecision := g.VP.GetString("Logs.TimePrecision")
	var recordTimeFormat string
	switch timePrecision {
	case "second":
		recordTimeFormat = g.DateFormat
	case "millisecond":
	...
	switch g.VP.GetString("Logs.TextFormat") {
	case "console":
	...
	// 检查日志文件保存路径
	fileName := g.VP.GetString("Logs.ProgramLog")
	if !filepath.IsAbs(fileName) {
		fileName = filepath.Join(g.BasePath, fileName)
	}
	_, err := myFileUtil.PathExistsAndCreat(filepath.Dir(fileName))
	if err != nil {
		log.Fatal("创建日志文件夹失败：" + err.Error())
		os.Exit(-1)
	}
	lumberJackLogger := &lumberjack.Logger{
		Filename:   fileName,                       //日志文件的位置
		MaxSize:    g.VP.GetInt("Logs.MaxSize"),    //在进行切割之前，日志文件的最大大小（以MB为单位）
		MaxBackups: g.VP.GetInt("Logs.MaxBackups"), //保留旧文件的最大个数
		MaxAge:     g.VP.GetInt("Logs.MaxAge"),     //保留旧文件的最大天数
		Compress:   g.VP.GetBool("Logs.Compress"),  //是否压缩/归档旧文件
	}
	...
	return zap.New(zapCore, zap.AddCaller(),zap.AddCallerSkip(1), zap.Hooks(entry), zap.AddStacktrace(zap.WarnLevel))
}
