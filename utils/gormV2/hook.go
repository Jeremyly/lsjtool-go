 ...
 * @LastEditTime: 2024-03-12 19:52:50
 * @Description:
...
// 		g.ZapLog.Warn("gorm Create 函数的参数必须是一个指针")
// 	}
// }
...
func UpdateBeforeHook(gormDB *gorm.DB) {
	if reflect.TypeOf(gormDB.Statement.Dest).Kind() == reflect.Struct {
		...
		g.ZapLog.Warn("gorm 的 Update、Save 函数的参数必须是一个指针(GinSkeleton ≥ v1.5.29 版本新增验证，为了完美支持 gorm 的所有回调函数,请在参数前面添加 & )")
	}
}
